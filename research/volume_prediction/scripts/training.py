...
def dnn(meta, compile_kwargs, **kwargs):
    ...
    for layer in range(1, kwargs.get("hid_layers_num", 2) + 1):
        model.add(
            Dense(
                kwargs.get(f"neurons_num_layer{layer}", 4),
...
class XGBSaveFoldsModels(callback.TrainingCallback):
    def __init__(self):
        self.cvboosters = None

    def after_training(self, model):
        self.cvboosters = [cvpack.bst for cvpack in model.cvfolds]
        return model
